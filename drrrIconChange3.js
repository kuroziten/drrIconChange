// ブックマークレット
// javascript:const room_name=document.getElementsByName("room_name")[0].value,iconList=["setton","gg","tanaka","kanra","bakyura","zaika","zawa","neko","neko2","twin","purple","orange","kai","usa","moza","girl","numakuro","bm","bear","rab","nyan","muff","muff_nyan"],pop=document.createElement("div");for(icon of(pop.style.position="fixed",pop.style.width="90vw",pop.style.height="80vh",pop.style.left="5vw",pop.style.top="10vh",pop.style.backgroundColor="white",document.body.append(pop),document.getElementsByClassName("message_box")[0].style.zIndex="0",iconList)){const e=document.createElement("img");e.src=`/css/icon_${icon}.png`;const n=icon;e.addEventListener("click",(async function(){await changeAction(n)})),pop.append(e)}async function changeAction(e){document.body.innerHTML="",$.cookie("durarara-like-chat1","",{path:"/",expires:-1});let n=await(await fetch("/logout/",{method:"POST"})).text(),o=document.createElement("div");o.style.display="none",o.innerHTML=n,document.body.append(o);const t=o.querySelector('input[name="token"]').value;o.remove();let a=new FormData;a.append("name","スイカ"),a.append("login","login"),a.append("language","ja-JP"),a.append("icon",e),a.append("token",t),n=await(await fetch("/",{method:"POST",body:a})).text(),o=document.createElement("div"),o.style.display="none",o.innerHTML=n,document.body.append(o);const c=o.querySelector("#zatsu").querySelectorAll(".rooms");let p=null;for(_room of c)if(_room.querySelector(".name").textContent==room_name){p=_room;break}const m=p.querySelector("input[type='hidden']").value;o.remove();const i=document.createElement("form");i.action="/room/",i.method="post";const r=document.createElement("input");r.name="id",r.value=m;const d=document.createElement("input");d.name="login",d.value="login",i.append(r),i.append(d),document.body.append(i),i.submit()};void(0);
